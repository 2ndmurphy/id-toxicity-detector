...
from transformers import BertTokenizer, BertModel, BertConfig # type: ignore
from transformers import AdamW, get_linear_schedule_with_warmup # type: ignore
#from google.colab import drive # type: ignore
import re, emoji # type: ignore

dataset_path = "ml-backend/model/hate_speech_HS2/data/"
...
if __name__ == '__main__':
    ...
    def preprocess_tweet(tweet):
        ...
        for word in tweet.split():
            if word[0] == '@' or word == '[username]':
                new_tweet.append('<user>') # replace with <user> token
            elif find_url(word) != []:
                new_tweet.append('<url>')  # replace with <url> token
    ...
    class Model(nn.Module):
        ...
        def forward(self, src, seg, mask_src):
            try:
                bert_output = self.bert(input_ids=src, token_type_ids=seg, attention_mask=mask_src)
                # top_vec, _ = self.bert(input_ids=src, token_type_ids=seg, attention_mask=mask_src)
                top_vec = bert_output.last_hidden_state
    ...
    # Instead of parsing arguments, create an object and set its attributes directly.
    class Args:
        def __init__(self):
            self.bert_model = 'indobertweet'
